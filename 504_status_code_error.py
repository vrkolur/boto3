...
def check_for_status_code(user_choice):
    ...
    try:
        if user_choice in valid_options:
            ...
            print("Invalid option. Please enter 401, 403, 409 or 504.")
            exit(0)
    except ValueError:
...
def handle_status_code(status_code,log_group_name,start_time,end_time,filter_pattern):
    actions = {
        504: lambda: filter_events(log_group_name,start_time,end_time,filter_pattern[0]),
    }
...
filter_pattern.append("filter @message like /status code 504/ | parse @message '\"orgID\":*,' as orgID | stats count(*) by orgID")
...
# Quick Tip:
    ...
    # 3. Please follow the syntax Carefully 
        # 1. SYNTAX: 'python file.py status_code trailing_time threshold aws_logGroupName' (allowed h,d and w only)
