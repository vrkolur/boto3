...
def filter_events(log_group_name, start_time = 1719014400000, end_time =1719100799000, filter_pattern = "fields @timestamp, @message, @logStream, @log| sort @timestamp desc| limit 10000"):
    ...
    start_query_response = client.start_query(
        ...
        endTime=end_time,    
        # queryString = 'filter @message like /401 Unauthorized/ | parse @message '"guid":"*"' as id| display @timestamp, id | limit 50'
        # queryString = "filter @message like /401 Unauthorized/ and @message like /guid/| parse @message '\"guid\":\"*\"' as id| stats count(*) by id"
...
# Get the response
response = filter_events(log_group_name,epoc_start_time, epoc_end_time, "filter @message like /401 Unauthorized/ and @message like /guid/ | parse @message '\"orgID\":*,' as orgId | parse @message '\"guid\":\"*' as guid | stats count(*) by guid, orgId")
